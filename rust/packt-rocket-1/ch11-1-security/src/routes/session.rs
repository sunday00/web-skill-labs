...
pub async fn new<'r>(flash: Option<FlashMessage<'_>>, csrf_token: CsrfToken) -> HtmlResponse {
    let flash_string = flash.map(|fl| format!("{}", fl.message())).unwrap_or_else(|| "".to_string());
    let context = context! { flash: flash_string, csrf_token: csrf_token};

    Ok(Template::render("sessions/new", context))
}

#[post("/login", format = "application/x-www-form-urlencoded", data = "<login_context>")]
pub async fn create<'r>(pool: &rocket::State<SqlitePool>, login_context: Form<Contextual<'r, Login<'r>>>, csrf_token: CsrfToken, cookies: &CookieJar<'_>) -> Result<Flash<Redirect>, Flash<Redirect>> {
    let login_error = || Flash::error(Redirect::to("/login"), "Cannot login");

    if login_context.value.is_none() { return Err(login_error()); }

    let login = login_context.value.as_ref().unwrap();
    csrf_token.verify(&login.authenticity_token).map_err(|_| login_error())?;

    let user = User::find_by_login(pool, login).await.map_err(|_| login_error())?;
