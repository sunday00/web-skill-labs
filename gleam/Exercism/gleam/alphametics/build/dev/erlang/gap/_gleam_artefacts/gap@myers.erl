...
-define(FILEPATH, "src/gap/myers.gleam").
-export([difference/2]).
...
-type edit(FQK) :: {eq, list(FQK)} | {del, list(FQK)} | {ins, list(FQK)}.
...
-type status(FQM) :: {done, list(edit(FQM))} |
    {next, list(path(FQM))} |
    {cont, path(FQM)}.

-file("src/gap/myers.gleam", 41).
-spec compact_reverse(list(edit(FQW)), list(edit(FQW))) -> list(edit(FQW)).
...
-file("src/gap/myers.gleam", 100).
-spec move_right(path(FRP)) -> path(FRP).
move_right(Path) ->
    case Path of
...
-file("src/gap/myers.gleam", 108).
-spec move_down(path(FRS)) -> path(FRS).
move_down(Path) ->
    case Path of
...
-file("src/gap/myers.gleam", 74).
-spec proceed_path(integer(), integer(), list(path(FRJ))) -> {path(FRJ),
    list(path(FRJ))}.
    ...
    case {Diag, Limit, Paths} of
        ...
        {_, _, _} ->
            erlang:error(#{gleam_error => panic,
                    message => <<"Unexpected case"/utf8>>,
                    file => <<?FILEPATH/utf8>>,
                    module => <<"gap/myers"/utf8>>,
                    function => <<"proceed_path"/utf8>>,
                    line => 96})
    end.

-file("src/gap/myers.gleam", 116).
-spec follow_snake(path(FRV)) -> status(FRV).
follow_snake(Path) ->
    case Path of
...
-file("src/gap/myers.gleam", 56).
-spec each_diagonal(integer(), integer(), list(path(FRD)), list(path(FRD))) -> status(FRD).
...
-file("src/gap/myers.gleam", 28).
-spec find_script(integer(), integer(), list(path(FQS))) -> list(edit(FQS)).
...
-spec difference(list(FQN), list(FQN)) -> list(edit(FQN)).
difference(List1, List2) ->
    Path = {path, 0, 0, List1, List2, []},
    find_script(0, erlang:length(List1) + erlang:length(List2), [Path]).
