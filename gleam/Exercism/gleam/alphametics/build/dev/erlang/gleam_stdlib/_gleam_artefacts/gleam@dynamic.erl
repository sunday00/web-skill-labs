...
-spec map_errors(
    {ok, CIU} | {error, list(decode_error())},
...
-spec optional(fun((dynamic_()) -> {ok, CKJ} | {error, list(decode_error())})) -> fun((dynamic_()) -> {ok,
        gleam@option:option(CKJ)} |
    {error, list(decode_error())}).
optional(Decode) ->
...
-spec do_any(
    list(fun((dynamic_()) -> {ok, CON} | {error, list(decode_error())}))
...
) -> fun((dynamic_()) -> {ok, {ok, CJR} | {error, CJT}} |
    {error, list(decode_error())}).
...
-spec list(fun((dynamic_()) -> {ok, CKE} | {error, list(decode_error())})) -> fun((dynamic_()) -> {ok,
        list(CKE)} |
    {error, list(decode_error())}).
list(Decoder_type) ->
    fun(Dynamic) ->
...
) -> fun((dynamic_()) -> {ok, gleam@option:option(CKX)} |
    {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {CMF, CMH}} | {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {CMK, CMM, CMO}} | {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {CMR, CMT, CMV, CMX}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, {CNA, CNC, CNE, CNG, CNI}} |
    {error, list(decode_error())}).
...
-spec tuple6(
    ...
    fun((dynamic_()) -> {ok, CNV} | {error, list(decode_error())})
) -> fun((dynamic_()) -> {ok, {CNL, CNN, CNP, CNR, CNT, CNV}} |
    {error, list(decode_error())}).
...
) -> fun((dynamic_()) -> {ok, gleam@dict:dict(CNY, COA)} |
    {error, list(decode_error())}).
...
-spec any(list(fun((dynamic_()) -> {ok, COJ} | {error, list(decode_error())}))) -> fun((dynamic_()) -> {ok,
        COJ} |
    {error, list(decode_error())}).
any(Decoders) ->
    do_any(Decoders).
...
-spec decode6(
    fun((CQF, CQG, CQH, CQI, CQJ, CQK) -> CQL),
...
-spec decode7(
    fun((CQT, CQU, CQV, CQW, CQX, CQY, CQZ) -> CRA),
...
-spec decode8(
    fun((CRJ, CRK, CRL, CRM, CRN, CRO, CRP, CRQ) -> CRR),
    fun((dynamic_()) -> {ok, CRJ} | {error, list(decode_error())}),
...
-spec decode9(
    fun((CSB, CSC, CSD, CSE, CSF, CSG, CSH, CSI, CSJ) -> CSK),
    fun((dynamic_()) -> {ok, CSB} | {error, list(decode_error())}),
