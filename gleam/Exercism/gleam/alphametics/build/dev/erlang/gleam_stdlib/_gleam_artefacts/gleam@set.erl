...
-opaque set(EFV) :: {set, gleam@dict:dict(EFV, list(nil))}.
...
-spec contains(set(EGG), EGG) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(EGI), EGI) -> set(EGI).
delete(Set, Member) ->
...
-spec fold(set(EGR), EGT, fun((EGT, EGR) -> EGT)) -> EGT.
...
-spec filter(set(EGU), fun((EGU) -> boolean())) -> set(EGU).
...
-spec drop(set(EHB), list(EHB)) -> set(EHB).
drop(Set, Disallowed) ->
...
-spec take(set(EHF), list(EHF)) -> set(EHF).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(EHN), set(EHN)) -> {set(EHN), set(EHN)}.
order(First, Second) ->
...
-spec intersection(set(EHS), set(EHS)) -> set(EHS).
...
-spec difference(set(EHW), set(EHW)) -> set(EHW).
...
-spec is_subset(set(EIA), set(EIA)) -> boolean().
...
-spec is_disjoint(set(EID), set(EID)) -> boolean().
...
-spec each(set(EIK), fun((EIK) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(EGD), EGD) -> set(EGD).
insert(Set, Member) ->
...
-spec from_list(list(EGO)) -> set(EGO).
from_list(Members) ->
...
-spec map(set(EGX), fun((EGX) -> EGZ)) -> set(EGZ).
map(Set, Fun) ->
...
-spec union(set(EHJ), set(EHJ)) -> set(EHJ).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(EIG), set(EIG)) -> set(EIG).
