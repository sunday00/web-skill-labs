...
-type message(KNF) :: {message, KNF} |
    {system, gleam@otp@system:system_message()} |
    {unexpected, gleam@dynamic:dynamic_()}.

-type next(KNG, KNH) :: {continue,
        KNH,
        gleam@option:option(gleam@erlang@process:selector(KNG))} |
    {stop, gleam@erlang@process:exit_reason()}.

-type init_result(KNI, KNJ) :: {ready, KNI, gleam@erlang@process:selector(KNJ)} |
    {failed, binary()}.

-type self(KNK, KNL) :: {self,
        gleam@otp@system:mode(),
        gleam@erlang@process:pid_(),
        KNK,
        gleam@erlang@process:subject(KNL),
        gleam@erlang@process:selector(message(KNL)),
        gleam@otp@system:debug_state(),
        fun((KNL, KNK) -> next(KNL, KNK))}.

-type spec(KNM, KNN) :: {spec,
        fun(() -> init_result(KNM, KNN)),
        integer(),
        fun((KNN, KNM) -> next(KNN, KNM))}.

-type start_error() :: init_timeout |
    {init_failed, gleam@erlang@process:exit_reason()} |
    {init_crashed, gleam@dynamic:dynamic_()}.

-type start_init_message(KNO) :: {ack,
        {ok, gleam@erlang@process:subject(KNO)} |
            {error, gleam@erlang@process:exit_reason()}} |
    {mon, gleam@erlang@process:process_down()}.

-file("src/gleam/otp/actor.gleam", 178).
-spec continue(KNV) -> next(any(), KNV).
continue(State) ->
    {continue, State, none}.
...
-spec with_selector(next(KNZ, KOA), gleam@erlang@process:selector(KNZ)) -> next(KNZ, KOA).
...
-spec selecting_system_messages(gleam@erlang@process:selector(message(KOL))) -> gleam@erlang@process:selector(message(KOL)).
...
-spec receive_message(self(any(), KOH)) -> message(KOH).
...
-spec init_selector(
    gleam@erlang@process:subject(KSQ),
    gleam@erlang@process:selector(KSQ)
) -> gleam@erlang@process:selector(message(KSQ)).
...
-spec initialise_actor(
    spec(any(), KPC),
    gleam@erlang@process:subject({ok, gleam@erlang@process:subject(KPC)} |
        {error, gleam@erlang@process:exit_reason()})
...
-spec start_spec(spec(any(), KPP)) -> {ok, gleam@erlang@process:subject(KPP)} |
    {error, start_error()}.
start_spec(Spec) ->
...
-spec start(KPV, fun((KPW, KPV) -> next(KPW, KPV))) -> {ok,
        gleam@erlang@process:subject(KPW)} |
    {error, start_error()}.
start(State, Loop) ->
    start_spec(
        {spec,
...
-spec send(gleam@erlang@process:subject(KQC), KQC) -> nil.
send(Subject, Msg) ->
...
-spec call(
    gleam@erlang@process:subject(KQE),
    fun((gleam@erlang@process:subject(KQG)) -> KQE),
    integer()
) -> KQG.
