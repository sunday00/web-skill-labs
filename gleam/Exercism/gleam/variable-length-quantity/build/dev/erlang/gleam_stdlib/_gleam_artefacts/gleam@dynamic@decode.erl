...
-opaque decoder(DOR) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {DOR, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(DOZ)) -> {ok, DOZ} |
...
-spec map(decoder(DSX), fun((DSX) -> DSZ)) -> decoder(DSZ).
...
-spec then(decoder(DTJ), fun((DTJ) -> decoder(DTL))) -> decoder(DTL).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(DTO), list(decoder(DTO))) -> decoder(DTO).
...
-spec recursive(fun(() -> decoder(DUE))) -> decoder(DUE).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(DST)) -> decoder(gleam@option:option(DST)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, DQU} | {error, DQU})
) -> {DQU, list(decode_error())}.
...
-spec collapse_errors(decoder(DTG), binary()) -> decoder(DTG).
...
-spec failure(DTY, binary()) -> decoder(DTY).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, DUA} | {error, DUA})
) -> decoder(DUA).
...
-spec fold_dict(
    {gleam@dict:dict(DSF, DSG), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {DSF, list(decode_error())}),
    fun((gleam@dynamic:dynamic_()) -> {DSG, list(decode_error())})
) -> {gleam@dict:dict(DSF, DSG), list(decode_error())}.
...
-spec dict(decoder(DRY), decoder(DSA)) -> decoder(gleam@dict:dict(DRY, DSA)).
dict(Key, Value) ->
...
-spec list(decoder(DRM)) -> decoder(list(DRM)).
list(Inner) ->
    {decoder,
        fun(Data) ->
...
-spec push_path({DPV, list(decode_error())}, list(any())) -> {DPV,
    list(decode_error())}.
...
-spec index(
    list(DPJ),
    list(DPJ),
    fun((gleam@dynamic:dynamic_()) -> {DPM, list(decode_error())}),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(DPJ)) -> {DPM, list(decode_error())})
) -> {DPM, list(decode_error())}.
...
-spec subfield(list(any()), decoder(DOU), fun((DOU) -> decoder(DOW))) -> decoder(DOW).
...
-spec at(list(any()), decoder(DPG)) -> decoder(DPG).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(DQE), fun((DQE) -> decoder(DQG))) -> decoder(DQG).
...
-spec optional_field(any(), DQK, decoder(DQK), fun((DQK) -> decoder(DQM))) -> decoder(DQM).
...
-spec optionally_at(list(any()), DQR, decoder(DQR)) -> decoder(DQR).
