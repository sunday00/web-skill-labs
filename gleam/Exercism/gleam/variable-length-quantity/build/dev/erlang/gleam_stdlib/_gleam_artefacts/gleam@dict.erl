...
-type dict(LF, LG) :: any() | {gleam_phantom, LF, LG}.
...
-spec to_list(dict(LP, LQ)) -> list({LP, LQ}).
to_list(Dict) ->
    maps:to_list(Dict).
...
-spec get(dict(MS, MT), MS) -> {ok, MT} | {error, nil}.
get(From, Get) ->
...
-spec has_key(dict(MG, any()), MG) -> boolean().
has_key(Dict, Key) ->
...
-spec from_list_loop(list({LZ, MA}), dict(LZ, MA)) -> dict(LZ, MA).
...
-spec from_list(list({LU, LV})) -> dict(LU, LV).
from_list(List) ->
    maps:from_list(List).

-file("src/gleam/dict.gleam", 223).
-spec reverse_and_concat(list(OI), list(OI)) -> list(OI).
...
-spec do_values_loop(list({any(), OS}), list(OS)) -> list(OS).
...
-spec values(dict(any(), ON)) -> list(ON).
values(Dict) ->
    maps:values(Dict).

-file("src/gleam/dict.gleam", 318).
-spec do_take_loop(dict(PW, PX), list(PW), dict(PW, PX)) -> dict(PW, PX).
...
-spec take(dict(PI, PJ), list(PI)) -> dict(PI, PJ).
...
-spec fold_inserts(list({QN, QO}), dict(QN, QO)) -> dict(QN, QO).
...
-spec merge(dict(QF, QG), dict(QF, QG)) -> dict(QF, QG).
...
-spec delete(dict(RA, RB), RA) -> dict(RA, RB).
delete(Dict, Key) ->
...
-spec drop(dict(RM, RN), list(RM)) -> dict(RM, RN).
...
-spec upsert(dict(RT, RU), RT, fun((gleam@option:option(RU)) -> RU)) -> dict(RT, RU).
upsert(Dict, Key, Fun) ->
...
-spec fold_loop(list({SF, SG}), SI, fun((SI, SF, SG) -> SI)) -> SI.
...
-spec fold(dict(SA, SB), SE, fun((SE, SA, SB) -> SE)) -> SE.
...
-spec filter(dict(OW, OX), fun((OW, OX) -> boolean())) -> dict(OW, OX).
