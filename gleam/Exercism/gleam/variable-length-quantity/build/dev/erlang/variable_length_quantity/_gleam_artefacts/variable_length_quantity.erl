...
-define(FILEPATH, "src/variable_length_quantity.gleam").
-export([encode/1, decode/1, main/0]).
-export_type([error/0]).

-type error() :: incomplete_sequence.

-file("src/variable_length_quantity.gleam", 40).
-spec do_encode(integer(), list(integer())) -> bitstring().
do_encode(N, Acc) ->
    echo({N, Acc}, 41),
    case N of
        0 ->
            _pipe = Acc,
            _pipe@1 = gleam@list:map(_pipe, fun(Byte) -> <<Byte:8>> end),
            gleam@list:fold(_pipe@1, <<>>, fun gleam@bit_array:append/2);

        _ ->
            echo(N, 50),
            Byte@1 = erlang:'band'(N, 16#7F),
            Next_bytes = case Acc of
                [] ->
                    [Byte@1];

                _ ->
                    [erlang:'bor'(Byte@1, 16#80) | Acc]
            end,
            do_encode(erlang:'bsr'(N, 7), Next_bytes)
    end.

-file("src/variable_length_quantity.gleam", 33).
-spec encode_single(integer()) -> bitstring().
encode_single(N) ->
    case N of
        0 ->
            <<0:8>>;
...
-file("src/variable_length_quantity.gleam", 11).
-spec encode(list(integer())) -> bitstring().
encode(Integers) ->
    _pipe = Integers,
    _pipe@1 = gleam@list:map(_pipe, fun encode_single/1),
    gleam@list:fold(_pipe@1, <<>>, fun gleam@bit_array:append/2).

-file("src/variable_length_quantity.gleam", 61).
-spec decode(bitstring()) -> {ok, list(integer())} | {error, error()}.
decode(String) ->
    erlang:error(#{gleam_error => todo,
            message => <<"`todo` expression evaluated. This code has not yet been implemented."/utf8>>,
            file => <<?FILEPATH/utf8>>,
            module => <<"variable_length_quantity"/utf8>>,
            function => <<"decode"/utf8>>,
            line => 62}).

-file("src/variable_length_quantity.gleam", 65).
-spec main() -> bitstring().
main() ->
    encode([16#80]).
