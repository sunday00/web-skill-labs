...
-opaque subject(IXM) :: {subject, pid_(), gleam@erlang:reference_()} |
    {gleam_phantom, IXM}.

-type do_not_leak() :: any().

-type selector(IXN) :: any() | {gleam_phantom, IXN}.
...
-type call_error(IXO) :: {callee_down, gleam@dynamic:dynamic_()} |
    call_timeout |
    {gleam_phantom, IXO}.
...
-spec 'receive'(subject(IXZ), integer()) -> {ok, IXZ} | {error, nil}.
...
-spec receive_forever(subject(IYD)) -> IYD.
...
-spec select(selector(IYH), integer()) -> {ok, IYH} | {error, nil}.
select(From, Within) ->
...
-spec select_forever(selector(IYL)) -> IYL.
select_forever(From) ->
...
-spec map_selector(selector(IYN), fun((IYN) -> IYP)) -> selector(IYP).
map_selector(A, B) ->
...
-spec merge_selector(selector(IYR), selector(IYR)) -> selector(IYR).
merge_selector(A, B) ->
...
-spec selecting_trapped_exits(selector(IYV), fun((exit_message()) -> IYV)) -> selector(IYV).
...
-spec selecting(selector(IYY), subject(IZA), fun((IZA) -> IYY)) -> selector(IYY).
...
-spec selecting_record2(
    selector(IZI),
    any(),
    fun((gleam@dynamic:dynamic_()) -> IZI)
) -> selector(IZI).
...
-spec selecting_record3(
    selector(IZM),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IZM)
) -> selector(IZM).
...
-spec selecting_record4(
    selector(IZQ),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IZQ)
) -> selector(IZQ).
...
-spec selecting_record5(
    selector(IZU),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IZU)
) -> selector(IZU).
...
-spec selecting_record6(
    selector(IZY),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> IZY)
) -> selector(IZY).
...
-spec selecting_record7(
    selector(JAC),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> JAC)
) -> selector(JAC).
...
-spec selecting_record8(
    selector(JAG),
    any(),
    fun((gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_(), gleam@dynamic:dynamic_()) -> JAG)
) -> selector(JAG).
...
-spec selecting_anything(selector(JAK), fun((gleam@dynamic:dynamic_()) -> JAK)) -> selector(JAK).
...
-spec deselecting(selector(IZD), subject(any())) -> selector(IZD).
...
-spec deselecting_process_down(selector(JAZ), process_monitor()) -> selector(JAZ).
...
-spec try_call(subject(JBC), fun((subject(JBE)) -> JBC), integer()) -> {ok, JBE} |
    {error, call_error(JBE)}.
...
-spec call(subject(JBJ), fun((subject(JBL)) -> JBJ), integer()) -> JBL.
...
-spec try_call_forever(subject(JBR), fun((subject(JBT)) -> JBR)) -> {ok, JBT} |
...
-spec call_forever(subject(JBN), fun((subject(JBP)) -> JBN)) -> JBP.
call_forever(Subject, Make_request) ->
    Response@1 = case try_call_forever(Subject, Make_request) of
...
-spec send_after(subject(JCA), integer(), JCA) -> timer().
