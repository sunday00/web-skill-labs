...
-spec map({ok, BYV} | {error, BYW}, fun((BYV) -> BYZ)) -> {ok, BYZ} |
    {error, BYW}.
map(Result, Fun) ->
    case Result of
        {ok, X} ->
...
-spec map_error({ok, BZC} | {error, BZD}, fun((BZD) -> BZG)) -> {ok, BZC} |
    {error, BZG}.
...
-spec flatten({ok, {ok, BZJ} | {error, BZK}} | {error, BZK}) -> {ok, BZJ} |
    {error, BZK}.
flatten(Result) ->
    case Result of
        {ok, X} ->
...
-spec 'try'({ok, BZR} | {error, BZS}, fun((BZR) -> {ok, BZV} | {error, BZS})) -> {ok,
        BZV} |
    {error, BZS}.
'try'(Result, Fun) ->
    case Result of
...
-spec then({ok, CAA} | {error, CAB}, fun((CAA) -> {ok, CAE} | {error, CAB})) -> {ok,
        CAE} |
    {error, CAB}.
then(Result, Fun) ->
    'try'(Result, Fun).
...
-spec unwrap({ok, CAJ} | {error, any()}, CAJ) -> CAJ.
...
-spec lazy_unwrap({ok, CAN} | {error, any()}, fun(() -> CAN)) -> CAN.
...
-spec unwrap_error({ok, any()} | {error, CAS}, CAS) -> CAS.
...
-spec unwrap_both({ok, CAV} | {error, CAV}) -> CAV.
unwrap_both(Result) ->
    case Result of
        {ok, A} ->
            A;
...
-spec 'or'({ok, CAY} | {error, CAZ}, {ok, CAY} | {error, CAZ}) -> {ok, CAY} |
    {error, CAZ}.
'or'(First, Second) ->
    case First of
        {ok, _} ->
...
-spec lazy_or({ok, CBG} | {error, CBH}, fun(() -> {ok, CBG} | {error, CBH})) -> {ok,
        CBG} |
    {error, CBH}.
...
-spec all(list({ok, CBO} | {error, CBP})) -> {ok, list(CBO)} | {error, CBP}.
all(Results) ->
    gleam@list:try_map(Results, fun(X) -> X end).

-file("src/gleam/result.gleam", 368).
-spec partition_loop(list({ok, CCD} | {error, CCE}), list(CCD), list(CCE)) -> {list(CCD),
    list(CCE)}.
...
-spec partition(list({ok, CBW} | {error, CBX})) -> {list(CBW), list(CBX)}.
partition(Results) ->
...
-spec replace({ok, any()} | {error, CCM}, CCP) -> {ok, CCP} | {error, CCM}.
...
-spec replace_error({ok, CCS} | {error, any()}, CCW) -> {ok, CCS} | {error, CCW}.
...
-spec values(list({ok, CCZ} | {error, any()})) -> list(CCZ).
values(Results) ->
