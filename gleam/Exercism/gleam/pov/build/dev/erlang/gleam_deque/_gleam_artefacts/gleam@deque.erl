...
-opaque deque(KIG) :: {deque, list(KIG), list(KIG)}.
...
-spec from_list(list(KIJ)) -> deque(KIJ).
from_list(List) ->
    {deque, [], List}.
...
-spec to_list(deque(KIM)) -> list(KIM).
to_list(Deque) ->
...
-spec push_back(deque(KIT), KIT) -> deque(KIT).
...
-spec push_front(deque(KIW), KIW) -> deque(KIW).
...
-spec pop_back(deque(KIZ)) -> {ok, {KIZ, deque(KIZ)}} | {error, nil}.
pop_back(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec pop_front(deque(KJE)) -> {ok, {KJE, deque(KJE)}} | {error, nil}.
pop_front(Deque) ->
    case Deque of
        {deque, [], []} ->
            {error, nil};
...
-spec reverse(deque(KJJ)) -> deque(KJJ).
reverse(Deque) ->
...
-spec is_logically_equal(deque(KJM), deque(KJM), fun((KJM, KJM) -> boolean())) -> boolean().
...
-spec is_equal(deque(KJU), deque(KJU)) -> boolean().
is_equal(A, B) ->
    check_equal(
        erlang:element(3, A),
        erlang:element(2, A),
