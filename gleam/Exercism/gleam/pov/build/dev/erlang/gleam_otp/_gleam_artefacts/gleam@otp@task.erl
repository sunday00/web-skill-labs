...
-opaque task(LOS) :: {task,
        gleam@erlang@process:pid_(),
        gleam@erlang@process:pid_(),
        gleam@erlang@process:subject(LOS)}.

-type await_error() :: timeout | {exit, gleam@dynamic:dynamic_()}.

-type message2(LOT, LOU) :: {m2_from_subject1, LOT} |
    {m2_from_subject2, LOU} |
    m2_timeout.

-type message3(LOV, LOW, LOX) :: {m3_from_subject1, LOV} |
    {m3_from_subject2, LOW} |
    {m3_from_subject3, LOX} |
    m3_timeout.

-type message4(LOY, LOZ, LPA, LPB) :: {m4_from_subject1, LOY} |
    {m4_from_subject2, LOZ} |
    {m4_from_subject3, LPA} |
    {m4_from_subject4, LPB} |
    m4_timeout.

-type message(LPC) :: {message, integer(), LPC} | message_timeout.
...
-spec try_await(task(LPH), integer()) -> {ok, LPH} | {error, await_error()}.
...
-spec await(task(LPL), integer()) -> LPL.
await(Task, Timeout) ->
...
-spec await_forever(task(LPP)) -> LPP.
await_forever(Task) ->
    assert_owner(Task),
    Selector = begin
        _pipe = gleam_erlang_ffi:new_selector(),
...
-spec try_await2_loop(
    gleam@erlang@process:selector(message2(LPZ, LQA)),
    gleam@option:option({ok, LPZ} | {error, await_error()}),
    gleam@option:option({ok, LQA} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, LPZ} | {error, await_error()}, {ok, LQA} | {error, await_error()}}.
...
-spec try_await2(task(LPR), task(LPT), integer()) -> {{ok, LPR} |
        {error, await_error()},
    {ok, LPT} | {error, await_error()}}.
...
-spec try_await3_loop(
    gleam@erlang@process:selector(message3(LRA, LRB, LRC)),
    gleam@option:option({ok, LRA} | {error, await_error()}),
    gleam@option:option({ok, LRB} | {error, await_error()}),
    gleam@option:option({ok, LRC} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, LRA} | {error, await_error()},
    {ok, LRB} | {error, await_error()},
    {ok, LRC} | {error, await_error()}}.
...
-spec try_await3(task(LQO), task(LQQ), task(LQS), integer()) -> {{ok, LQO} |
        {error, await_error()},
    {ok, LQQ} | {error, await_error()},
    {ok, LQS} | {error, await_error()}}.
...
-spec try_await4_loop(
    gleam@erlang@process:selector(message4(LSM, LSN, LSO, LSP)),
    gleam@option:option({ok, LSM} | {error, await_error()}),
    gleam@option:option({ok, LSN} | {error, await_error()}),
    gleam@option:option({ok, LSO} | {error, await_error()}),
    gleam@option:option({ok, LSP} | {error, await_error()}),
    gleam@erlang@process:timer()
) -> {{ok, LSM} | {error, await_error()},
    {ok, LSN} | {error, await_error()},
    {ok, LSO} | {error, await_error()},
    {ok, LSP} | {error, await_error()}}.
...
-spec try_await4(task(LRW), task(LRY), task(LSA), task(LSC), integer()) -> {{ok,
            LRW} |
        {error, await_error()},
    {ok, LRY} | {error, await_error()},
    {ok, LSA} | {error, await_error()},
    {ok, LSC} | {error, await_error()}}.
...
-spec dict_to_list(gleam@dict:dict(integer(), LUF), integer(), LUF) -> list(LUF).
...
-spec try_await_all_loop(
    gleam@dict:dict(integer(), {ok, LTV} | {error, await_error()}),
    integer(),
    gleam@erlang@process:timer(),
    gleam@erlang@process:selector(message(LTV))
) -> list({ok, LTV} | {error, await_error()}).
...
-spec try_await_all(list(task(LTP)), integer()) -> list({ok, LTP} |
    {error, await_error()}).
