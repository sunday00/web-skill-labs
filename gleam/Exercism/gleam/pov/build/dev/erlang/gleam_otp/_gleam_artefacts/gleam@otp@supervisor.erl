...
-type spec(LCJ, LCK) :: {spec,
        LCJ,
        integer(),
        integer(),
        fun((children(LCJ)) -> children(LCK))}.

-opaque children(LCL) :: {ready, starter(LCL)} | {failed, child_start_error()}.

-opaque child_spec(LCM, LCN, LCO) :: {child_spec,
        fun((LCN) -> {ok, gleam@erlang@process:subject(LCM)} |
            {error, gleam@otp@actor:start_error()}),
        fun((LCN, gleam@erlang@process:subject(LCM)) -> LCO)}.
...
-spec start_child(child_spec(any(), LCV, LCW), LCV) -> {ok, child(LCW)} |
    {error, child_start_error()}.
...
-spec perform_instruction_for_child(
    LDJ,
    instruction(),
    child_spec(any(), LDJ, LDL),
    child(LDL)
) -> {ok, {child(LDL), instruction()}} | {error, child_start_error()}.
...
-spec start_and_add_child(starter(LEC), child_spec(any(), LEC, LEF)) -> children(LEF).
...
-spec add(children(LEK), child_spec(any(), LEK, LEN)) -> children(LEN).
...
-spec supervisor(
    fun((LES) -> {ok, gleam@erlang@process:subject(LET)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(LET, LES, LES).
supervisor(Start) ->
...
-spec worker(
    fun((LFA) -> {ok, gleam@erlang@process:subject(LFB)} |
        {error, gleam@otp@actor:start_error()})
) -> child_spec(LFB, LFA, LFA).
worker(Start) ->
    {child_spec, Start, fun(Argument, _) -> Argument end}.
...
-spec returning(
    child_spec(LFI, LFJ, any()),
    fun((LFJ, gleam@erlang@process:subject(LFI)) -> LFP)
) -> child_spec(LFI, LFJ, LFP).
returning(Child, Updater) ->
    {child_spec, erlang:element(2, Child), Updater}.

-file("src/gleam/otp/supervisor.gleam", 248).
-spec init(spec(any(), LFU)) -> gleam@otp@actor:init_result(state(LFU), message()).
init(Spec) ->
...
-spec handle_exit(gleam@erlang@process:pid_(), state(LGA)) -> gleam@otp@actor:next(message(), state(LGA)).
...
-spec loop(message(), state(LGF)) -> gleam@otp@actor:next(message(), state(LGF)).
loop(Message, State) ->
    case Message of
        {exit, Exit_message} ->
