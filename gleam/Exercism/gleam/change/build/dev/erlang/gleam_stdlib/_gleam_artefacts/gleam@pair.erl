...
-spec map_first({YR, YS}, fun((YR) -> YT)) -> {YT, YS}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({YU, YV}, fun((YV) -> YW)) -> {YU, YW}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
