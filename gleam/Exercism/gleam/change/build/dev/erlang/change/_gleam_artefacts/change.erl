...
-define(FILEPATH, "src/change.gleam").
-export([find_fewest_coins/2, main/0]).
-export_type([error/0]).

-type error() :: impossible_target.

-file("src/change.gleam", 24).
-spec reducer(
    list(list(integer())),
    list(integer()),
    list(integer()),
...
reducer(Total_res, Acc, Rem, Target, Initial_coins) ->
    echo({Total_res, Acc, Rem}, 31),
    case Rem of
        [F | R] ->
            Sum = begin
                _pipe = Acc,
                _pipe@1 = gleam@list:reduce(_pipe, fun(Ac, Cu) -> Ac + Cu end),
...
-file("src/change.gleam", 8).
-spec find_fewest_coins(list(integer()), integer()) -> {ok, list(integer())} |
    {error, error()}.
find_fewest_coins(Coins, Target) ->
    case Target of
        ...
        _ ->
            Coins@1 = begin
                _pipe = Coins,
                _pipe@1 = gleam@list:filter(_pipe, fun(El) -> El < Target end),
                lists:reverse(_pipe@1)
            end,
            reducer([], [], Coins@1, Target, Coins@1),
            erlang:error(#{gleam_error => todo,
                    message => <<"`todo` expression evaluated. This code has not yet been implemented."/utf8>>,
                    file => <<?FILEPATH/utf8>>,
                    module => <<"change"/utf8>>,
                    function => <<"find_fewest_coins"/utf8>>,
                    line => 19})
    end.

-file("src/change.gleam", 72).
-spec main() -> {ok, list(integer())} | {error, error()}.
main() ->
    echo(find_fewest_coins([2, 5, 10, 20, 50], 21), 73).
