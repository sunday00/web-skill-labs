...
-spec to_result(option(HB), HE) -> {ok, HB} | {error, HE}.
to_result(Option, E) ->
    case Option of
        {some, A} ->
            {ok, A};
...
-spec from_result({ok, HH} | {error, any()}) -> option(HH).
from_result(Result) ->
    case Result of
        {ok, A} ->
            {some, A};
...
-spec lazy_unwrap(option(HO), fun(() -> HO)) -> HO.
...
-spec map(option(HQ), fun((HQ) -> HS)) -> option(HS).
map(Option, Fun) ->
    case Option of
        {some, X} ->
            {some, Fun(X)};
...
-spec flatten(option(option(HU))) -> option(HU).
flatten(Option) ->
    case Option of
        {some, X} ->
            X;
...
-spec then(option(HY), fun((HY) -> option(IA))) -> option(IA).
then(Option, Fun) ->
    case Option of
        {some, X} ->
            Fun(X);
...
-spec 'or'(option(ID), option(ID)) -> option(ID).
'or'(First, Second) ->
    case First of
        {some, _} ->
            First;
...
-spec lazy_or(option(IH), fun(() -> option(IH))) -> option(IH).
...
-spec values_loop(list(option(IP)), list(IP)) -> list(IP).
