...
-spec decode_bits(
    bitstring(),
    fun((gleam@dynamic:dynamic_()) -> {ok, EWC} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, EWC} | {error, decode_error()}.
...
-spec do_decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, EVW} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, EVW} | {error, decode_error()}.
...
-spec decode(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, EVK} |
        {error, list(gleam@dynamic:decode_error())})
) -> {ok, EVK} | {error, decode_error()}.
decode(Json, Decoder) ->
    do_decode(Json, Decoder).
...
-spec parse_bits(bitstring(), gleam@dynamic@decode:decoder(EWG)) -> {ok, EWG} |
    {error, decode_error()}.
...
-spec do_parse(binary(), gleam@dynamic@decode:decoder(EVS)) -> {ok, EVS} |
    {error, decode_error()}.
...
-spec parse(binary(), gleam@dynamic@decode:decoder(EVO)) -> {ok, EVO} |
    {error, decode_error()}.
parse(Json, Decoder) ->
    do_parse(Json, Decoder).
...
-spec nullable(gleam@option:option(EWM), fun((EWM) -> json())) -> json().
...
-spec array(list(EWQ), fun((EWQ) -> json())) -> json().
