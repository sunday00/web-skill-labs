...
-file("src/gleam/erlang/node.gleam", 10).
?DOC(" Return the current node.\n").
-spec self() -> node_().
self() ->
    erlang:node().

-file("src/gleam/erlang/node.gleam", 23).
?DOC(
...
-file("src/gleam/erlang/node.gleam", 44).
?DOC(
...
-file("src/gleam/erlang/node.gleam", 51).
?DOC(
...
-file("src/gleam/erlang/node.gleam", 62).
