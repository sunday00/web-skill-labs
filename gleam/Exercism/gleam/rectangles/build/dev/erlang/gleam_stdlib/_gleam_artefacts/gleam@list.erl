...
-type continue_or_stop(AAA) :: {continue, AAA} | {stop, AAA}.
...
-spec reverse_and_prepend(list(AAK), list(AAK)) -> list(AAK).
...
-spec contains(list(AAQ), AAQ) -> boolean().
contains(List, Elem) ->
    case List of
        [] ->
            false;
...
-spec first(list(AAS)) -> {ok, AAS} | {error, nil}.
first(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec rest(list(AAW)) -> {ok, list(AAW)} | {error, nil}.
rest(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec update_group(fun((ABH) -> ABI)) -> fun((gleam@dict:dict(ABI, list(ABH)), ABH) -> gleam@dict:dict(ABI, list(ABH))).
update_group(F) ->
...
-spec filter_loop(list(ABS), fun((ABS) -> boolean()), list(ABS)) -> list(ABS).
...
-spec filter(list(ABP), fun((ABP) -> boolean())) -> list(ABP).
...
-spec filter_map(list(ABW), fun((ABW) -> {ok, ABY} | {error, any()})) -> list(ABY).
filter_map(List, Fun) ->
    filter_map_loop(List, Fun, []).

-file("src/gleam/list.gleam", 384).
-spec map_loop(list(ACP), fun((ACP) -> ACR), list(ACR)) -> list(ACR).
...
-spec map(list(ACL), fun((ACL) -> ACN)) -> list(ACN).
map(List, Fun) ->
    map_loop(List, Fun, []).

-file("src/gleam/list.gleam", 411).
-spec map2_loop(list(ADA), list(ADC), fun((ADA, ADC) -> ADE), list(ADE)) -> list(ADE).
...
-spec map2(list(ACU), list(ACW), fun((ACU, ACW) -> ACY)) -> list(ACY).
...
-spec flatten_loop(list(list(AFN)), list(AFN)) -> list(AFN).
...
-spec flatten(list(list(AFS))) -> list(AFS).
flatten(Lists) ->
    flatten_loop(Lists, []).
...
-spec count(list(AAF), fun((AAF) -> boolean())) -> integer().
...
-spec group(list(ABB), fun((ABB) -> ABD)) -> gleam@dict:dict(ABD, list(ABB)).
group(List, Key) ->
...
-spec map_fold(list(ADH), ADJ, fun((ADJ, ADH) -> {ADJ, ADK})) -> {ADJ,
    list(ADK)}.
...
-spec pop_loop(list(BFM), fun((BFM) -> boolean()), list(BFM)) -> {ok,
        {BFM, list(BFM)}} |
    {error, nil}.
...
-spec pop(list(AMK), fun((AMK) -> boolean())) -> {ok, {AMK, list(AMK)}} |
    {error, nil}.
pop(List, Is_desired) ->
...
-spec pop_map_loop(
    list(ANC),
    fun((ANC) -> {ok, ANE} | {error, any()}),
    list(ANC)
) -> {ok, {ANE, list(ANC)}} | {error, nil}.
...
-spec pop_map(list(AMT), fun((AMT) -> {ok, AMV} | {error, any()})) -> {ok,
        {AMV, list(AMT)}} |
    {error, nil}.
...
-spec key_pop_loop(list({ANS, ANT}), ANS, list({ANS, ANT})) -> {ok,
        {ANT, list({ANS, ANT})}} |
    {error, nil}.
...
-spec key_pop(list({ANM, ANN}), ANM) -> {ok, {ANN, list({ANM, ANN})}} |
    {error, nil}.
key_pop(List, Key) ->
    key_pop_loop(List, Key, []).

-file("src/gleam/list.gleam", 1785).
-spec key_set_loop(list({AOD, AOE}), AOD, AOE, list({AOD, AOE})) -> list({AOD,
    AOE}).
...
-spec each(list(AOI), fun((AOI) -> any())) -> nil.
each(List, F) ->
    case List of
        [] ->
            nil;
...
-spec try_each(list(AOL), fun((AOL) -> {ok, any()} | {error, AOO})) -> {ok, nil} |
    {error, AOO}.
try_each(List, Fun) ->
    case List of
        [] ->
...
-spec partition_loop(list(BHX), fun((BHX) -> boolean()), list(BHX), list(BHX)) -> {list(BHX),
    list(BHX)}.
...
-spec partition(list(AOT), fun((AOT) -> boolean())) -> {list(AOT), list(AOT)}.
...
-spec permutations(list(APC)) -> list(list(APC)).
permutations(List) ->
    case List of
        [] ->
            [[]];
...
-spec window_loop(list(list(APK)), list(APK), integer()) -> list(list(APK)).
...
-spec window(list(APG), integer()) -> list(list(APG)).
window(List, N) ->
    case N =< 0 of
        true ->
            [];
...
-spec window_by_2(list(APQ)) -> list({APQ, APQ}).
window_by_2(List) ->
...
-spec drop_while(list(APT), fun((APT) -> boolean())) -> list(APT).
...
-spec take_while_loop(list(APZ), fun((APZ) -> boolean()), list(APZ)) -> list(APZ).
...
-spec chunk_loop(list(AQI), fun((AQI) -> AQK), AQK, list(AQI), list(list(AQI))) -> list(list(AQI)).
...
-spec chunk(list(AQD), fun((AQD) -> any())) -> list(list(AQD)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-spec sized_chunk(list(AQQ), integer()) -> list(list(AQQ)).
...
-spec reduce(list(ARB), fun((ARB, ARB) -> ARB)) -> {ok, ARB} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(ARJ), ARL, list(ARL), fun((ARL, ARJ) -> ARL)) -> list(ARL).
...
-spec scan(list(ARF), ARH, fun((ARH, ARF) -> ARH)) -> list(ARH).
...
-spec last(list(ARO)) -> {ok, ARO} | {error, nil}.
last(List) ->
...
-spec combinations(list(ARS), integer()) -> list(list(ARS)).
...
-spec combination_pairs_loop(list(ARZ)) -> list(list({ARZ, ARZ})).
...
-spec transpose(list(list(ASH))) -> list(list(ASH)).
...
-spec interleave(list(list(ASD))) -> list(ASD).
interleave(List) ->
    _pipe = transpose(List),
    flatten(_pipe).

-file("src/gleam/list.gleam", 2304).
-spec shuffle_pair_unwrap_loop(list({float(), ASP}), list(ASP)) -> list(ASP).
...
-spec do_shuffle_by_pair_indexes(list({float(), AST})) -> list({float(), AST}).
...
-spec shuffle(list(ASM)) -> list(ASM).
shuffle(List) ->
    _pipe = List,
...
-spec max(list(ASW), fun((ASW, ASW) -> gleam@order:order())) -> {ok, ASW} |
    {error, nil}.
max(List, Compare) ->
...
-spec sample(list(ATA), integer()) -> list(ATA).
sample(List, K) ->
    case K =< 0 of
        true ->
            [];
