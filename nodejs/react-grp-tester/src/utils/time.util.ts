...
export const time = (dateTime?: string | Date | DAYJS.Dayjs | number) => {
  if (dateTime)
...
export const week = (date: string) => {
  ...
  return `${point.format('M')}M-${
    point.week() === 1
      ? point.subtract(1, 'w').week() + 1
      : Number(point.format('w')) - Number(point.startOf('M').format('w')) + 1
  }W`
}
...
export const LocalTime = (dateTime?: string | Date | DAYJS.Dayjs | number) =>
